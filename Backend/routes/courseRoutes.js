...
// Request tracking system to prevent duplicate Gemini API calls
...
const requestStats = {
    ...
    byRoute: {
        '/generate': { total: 0, duplicates: 0 },
        '/benefits': { total: 0, duplicates: 0 },
        '/topic': { total: 0, duplicates: 0 },
        '/chat': { total: 0, duplicates: 0 }
    },
    // Reset stats every hour to avoid memory growth
    resetTime: Date.now() + 3600000
};

/**
 * Generate a unique key for a request based on its parameters
 * @param {string} route - The route being called (e.g., '/topic')
 * @param {Object} params - The parameters of the request
 * @returns {string} A unique key for the request
 */
const getRequestKey = (route, params) => {
    switch(route) {
        case '/topic':
            // For topic requests, use the course name and section number
            return `topic_${params.topicdata?.Course_Name || 'unknown'}_${params.topicdata?.section || 'unknown'}`;
        case '/generate':
            // For generate requests, use the course name or prompt
            return `generate_${params.Course_Name || params.prompt || 'unknown'}`;
        case '/benefits':
            // For benefits requests, use the course name
            return `benefits_${params.Course_Name || 'unknown'}`;
        case '/chat':
            // For chat requests, use a combination of parameters
            // Include conversation history length to differentiate conversations
            const historyHash = params.conversationHistory ? 
                `_history${params.conversationHistory.length}` : '';
            return `chat_${params.course || 'general'}_${params.section || 'all'}_${params.message.substring(0, 50)}${historyHash}`;
        default:
            // For any other request, stringify the parameters
            return `${route}_${JSON.stringify(params)}`;
    }
};

/**
 * Track a request and prevent duplicate Gemini API calls
 * @param {string} route - The route being called
 * @param {Object} params - The request parameters
 * @param {Function} processRequest - Function to process the request if not duplicate
 * @returns {Promise<any>} - The result of the request
 */
const trackRequest = async (route, params, processRequest) => {
    const requestKey = getRequestKey(route, params);
    
    // Update stats
    if (Date.now() > requestStats.resetTime) {
        // Reset stats every hour
        Object.keys(requestStats.byRoute).forEach(r => {
            requestStats.byRoute[r] = { total: 0, duplicates: 0 };
        });
        requestStats.totalRequests = 0;
        requestStats.duplicatePrevented = 0;
        requestStats.resetTime = Date.now() + 3600000;
    }
    
    requestStats.totalRequests++;
    if (requestStats.byRoute[route]) {
        requestStats.byRoute[route].total++;
    }
    
    // Check if an identical request is already in progress
    if (ongoingRequests.has(requestKey)) {
        requestStats.duplicatePrevented++;
        if (requestStats.byRoute[route]) {
            requestStats.byRoute[route].duplicates++;
        }
        
        console.log(`[DUPLICATE] Request detected for ${requestKey}.`);
        console.log(`Stats: Prevented ${requestStats.duplicatePrevented}/${requestStats.totalRequests} duplicate requests (${((requestStats.duplicatePrevented/requestStats.totalRequests)*100).toFixed(1)}%)`);
    ...
    const requestPromise = (async () => {
        ...
        try {
            ...
            console.log(`[COMPLETED] Request ${requestKey} took ${duration}ms`);
            return result;
        } catch (error) {
            console.error(`[ERROR] Request ${requestKey} failed: ${error.message}`);
            throw error;
        } finally {
            // Clean up tracking when done (regardless of success or failure)
            setTimeout(() => {
                ongoingRequests.delete(requestKey);
            }, 100); // Small delay to handle potential near-simultaneous requests
        }
    })();
    
    // Track this request
    ongoingRequests.set(requestKey, requestPromise);
    console.log(`[NEW] Tracking request: ${requestKey}`);
    console.log(`Active requests: ${ongoingRequests.size}, Route stats for ${route}: ${requestStats.byRoute[route]?.total || 0} total, ${requestStats.byRoute[route]?.duplicates || 0} duplicates`);
...
router.post('/generate', async (req, res) => {
    try {
        ...
        if (!prompt && !Course_Name) {
            return res.status(400).json({ error: 'Prompt or Course_Name is required' });
        }

        // Use the request tracking system to prevent duplicate Gemini API calls
        const result = await trackRequest('/generate', req.body, async () => {
            // Check if course exists by name (if provided)
            if (Course_Name) {
            ...
            // STEP 0: Check if the course already exists by prompt
...
router.post('/benefits', async (req, res) => {
    try {
        ...
        // Use the request tracking system to prevent duplicate Gemini API calls
        const result = await trackRequest('/benefits', req.body, async () => {
            const course_name_with_spaces = Course_Name.replace(/-/g, ' ');
            const existingCourse = await Course.findOne({ Course_Name: course_name_with_spaces });
            ...
            if (existingCourse.benefits && existingCourse.benefits.length > 0) {
                return existingCourse.benefits;
            }
...
router.post('/topic', async (req, res) => {
    try {
        ...
        // Use the request tracking system to prevent duplicate Gemini API calls
        const result = await trackRequest('/topic', topicdata, async () => {
...
router.post('/chat', async (req, res) => {
    try {
        ...
        // Use the request tracking system to prevent duplicate Gemini API calls
        // Note: For chat, we want identical questions to share processing, but
        // different conversations should be treated separately
        const result = await trackRequest('/chat', req.body, async () => {
            ...
            const chatResult = await generateContent(chatPrompt, 'FLASH');
...
/**
 * Get duplicate request prevention statistics
 * GET /request-stats
 */
router.get('/request-stats', (req, res) => {
    // Calculate percentages for better reporting
    const stats = {
        ...requestStats,
        activeRequests: ongoingRequests.size,
        duplicatePercentage: requestStats.totalRequests > 0 
            ? ((requestStats.duplicatePrevented / requestStats.totalRequests) * 100).toFixed(2) + '%' 
            : '0%',
        byRoute: {}
    };
    
    // Add percentage data for each route
    Object.keys(requestStats.byRoute).forEach(route => {
        const routeStats = requestStats.byRoute[route];
        stats.byRoute[route] = {
            ...routeStats,
            duplicatePercentage: routeStats.total > 0 
                ? ((routeStats.duplicates / routeStats.total) * 100).toFixed(2) + '%'
                : '0%'
        };
    });
    ...
    stats.timeInfo = {
        ...
        statsResetAt: new Date(requestStats.resetTime).toISOString(),
        secondsUntilReset: Math.max(0, Math.floor((requestStats.resetTime - Date.now()) / 1000))
    };
    
    // Add active request details (with timestamps)
    stats.currentlyProcessing = Array.from(ongoingRequests.keys()).map(key => {
...
/**
 * Check content generation status for a specific course section
 * POST /check-generation-status
 */
router.post('/check-generation-status', async (req, res) => {
    try {
        ...
        if (!courseName || !sectionId) {
            return res.status(400).json({ 
                ...
                message: 'Both courseName and sectionId are required'
            });
        }
        
        // Convert courseName to the format used in MongoDB (spaces instead of hyphens)
        const course_name_with_spaces = courseName.replace(/-/g, ' ');
        ...
        // Check if the requested section has content
        const existingSection = existingCourse.generated_content.find(
            item => item.section === parseInt(sectionId)
        );
        
        if (existingSection && existingSection.content && existingSection.content.length > 0) {
            // Get section info for the notification
            const sectionInfo = existingCourse.learning_path.find(
                item => item.section === parseInt(sectionId)
            );
            
            // Content exists, return success
            return res.json({
                isGenerated: true,
                course: {
                    name: existingCourse.Course_Name,
                ...
                section: {
                    id: sectionId,
                    name: sectionInfo?.topic_name || `Section ${sectionId}`
                },
            ...
            return res.json({
                isGenerated: false,
                message: 'Content generation is still in progress'
            });
        }
    } catch (error) {
